...
$this->title = 'Доходы по вводу по мерчантам';
?>
    
<h4 class="text-center">Доходы по вводу в разрезе мерчантов</h4>
...
<?php 
    ...
    $table = '<table class="table table-striped table-bordered"><tr><th>Валюта</th><th>ID мерчанта</th><th>Мерчант</th><th>Зачислено мерчантам</th>
        <th>Поступило в ПС</th><th>Комиссия ПС</th><th>Конв. комиссия</th><th>Доход</th><th>Прибыль</th></tr>';	
    foreach ($result1 as $row) {
	$table .= '<tr><td class="text-center">'.$row['curr_ps'].'</td>';
	$table .= '<td class="text-left">'.$row['merch_id'].'</td>';
        $table .= '<td class="text-left">'.$row['merch_name'].'</td>';
